...
function library:saveConfig()
    ...
    local json = game:GetService("HttpService"):JSONEncode(jig)
    writefile(name .. ".json", json)
end
function library:refreshConfigs()
    local directory = "alora/" .. tostring(game.GameId) .. "/"
    local configs = {}

    if isfolder(directory) then
        for _, fileName in ipairs(listfiles(directory)) do
            if fileName:match("%.cfg$") then
    ...
    library.configs = configs

    if library.updateConfigList then
        library:updateConfigList(configs)
    end
end

function library:loadConfig()
    local name = library.flags["selected_config"]
    local filePath = "alora/"..tostring(game.GameId).."/"..name..".cfg"
    ...
    local jig = game:GetService("HttpService"):JSONDecode(json)

    for i,v in next, jig do
        if typeof(library.flags[i]) == "Color3" and typeof(v) == "table" then
            library.flags[i] = Color3.new(unpack(v))
        elseif typeof(library.flags[i]) == "EnumItem" and typeof(v) == "table" then
            library.flags[i] = Enum[v[1]][v[2]]
        else
            library.flags[i] = v
        end
    end

    library:notify("Config loaded successfully!")
end
